...
def test_wellstring_layers_int():
    # int
    ml = fs.Model3D(z=[0, -1, -2, -3])
    ws = fs.WellStringBase(ml, [(0, 0), (10, 0), (0, 10)], layers=2)
...
def test_wellstring_layers_list():
    # list
    ml = fs.Model3D(z=[0, -1, -2, -3])
    ws = fs.WellStringBase(ml, [(0, 0), (10, 0), (0, 10)], layers=[1, 2])
...
def test_wellstring_layers_1d_array():
    # 1d array
    ml = fs.Model3D(z=[0, -1, -2, -3])
    ws = fs.WellStringBase(ml, [(0, 0), (10, 0), (0, 10)], layers=np.arange(1, 4))
