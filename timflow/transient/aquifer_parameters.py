...
def param_maq(
    ...
    if topboundary[:3] == "con":
        ...
        assert len(beta) == naq, "Error: Length of beta needs to be " + str(naq)
        Haq = H[1::2]
        Hll = H[::2]
        # layertype
        nlayers = len(z) - 1
        ltype = np.array(nlayers * ["a"])
        ltype[0::2] = "l"
    return kaq, Haq, Hll, c, Saq, Sll, beta, poraq, porll, ltype
...
def param_3d(
    ...
    return kaq, Haq, Hll, c, Saq, Sll, beta, poraq, porll, ltype, z
